...
>>>>>>> a88738f1bce31aa7d6b9aaf9c359bbb00a29d2e1
        ...
        const envPromise = new Promise((resolve) => {
            loader.load(path, (gltf) => {
                ...
                this.environmentBasePosition = this.environment.position.clone();
        ...
        const targetPromise = targetPath ? new Promise((resolve) => {
            loader.load(targetPath, (gltf) => {
                ...
                const gridX = window.mevcutHedef ? window.mevcutHedef.x : 2; 
                const gridZ = window.mevcutHedef ? window.mevcutHedef.z : 2; 
                ...
                this.targetBasePosition = this.target.position.clone();
    ...
    update(pos, rot, targetPos) {
        if (this.character) {
            ...
            if (this.environment) {
                const rayOrigin = new THREE.Vector3(0, 20, 0); // karakter sabit
            ...
            this.character.position.set(0, this.currentHoverY, 0); // karakter merkezde
            
            this.character.rotation.y = THREE.MathUtils.degToRad(rot); 
            
            if (this.shadowDisc) {
                this.shadowDisc.position.set(0, groundY + 0.05, 0); // gölge merkezde
            }

            if (this.environment && this.environmentBasePosition) {
                this.environment.position.x = this.environmentBasePosition.x - worldX;
                this.environment.position.z = this.environmentBasePosition.z - worldZ;
            }

            if (this.target && this.targetBasePosition) {
                this.target.position.set(
                    this.targetBasePosition.x - worldX,
                    this.targetBasePosition.y,
                    this.targetBasePosition.z - worldZ
                );
            }

            const cameraDistance = 6;
            const camAngle = THREE.MathUtils.degToRad(rot);
            const camX = Math.sin(camAngle + Math.PI) * cameraDistance;
            const camZ = Math.cos(camAngle + Math.PI) * cameraDistance;
            this.camera.position.set(camX, this.currentHoverY + 6, camZ);
            this.camera.lookAt(0, this.currentHoverY, 0);
            this.camera.rotation.y = THREE.MathUtils.degToRad(rot); // kamera dönmesi için
        }
        ...
        if (this.target && this.environment) {
            const gridGoalX = targetPos ? targetPos.x : 2;
            const gridGoalZ = targetPos ? targetPos.z : 2;
