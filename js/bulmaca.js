...
// Her çağ için 3 farklı soru eklendi. Sistem rastgele birini seçecek.
        ...
        sorular: [
            {
                ...
                cevap: "sıralama",
                ipucu: "S... ile başlayan, şeyleri art arda dizmek anlamına gelen bir kelime."
            },
            {
...
let mevcutSeviyeIndeksi = 0;
let quizModalInstance = null;
let aktifRastgeleSoru = null; // Rastgele seçilen soruyu hafızada tutmak için
...
function modalIceriginiHazirla() {
    ...
    document.getElementById("cevap-input").addEventListener("keypress", function(event) {
        if (event.key === "Enter") {
...
// === 3. 2. KİŞİ (MELİKE) İLE BAĞLANTI ===
window.soruEkraniAc = function() {
    console.log("Motor parça buldu! Soru ekranı tetiklendi.");
    
    if (mevcutSeviyeIndeksi >= bulmacaVeritabani.length) return;

    if (document.getElementById("cevap-input") === null) {
        modalIceriginiHazirla();
    }

    const aktifSeviye = bulmacaVeritabani[mevcutSeviyeIndeksi];
    
    // RASTGELE SORU SEÇİMİ (O çağdaki sorulardan birini seçer)
    const rastgeleSayi = Math.floor(Math.random() * aktifSeviye.sorular.length);
    aktifRastgeleSoru = aktifSeviye.sorular[rastgeleSayi];
    
    // Veritabanındaki rastgele soruyu ekrana bas
    ...
    if (!quizModalInstance) {
        quizModalInstance = new bootstrap.Modal(modalElement, { backdrop: 'static', keyboard: false });
    }
...
window.cevabiKontrolEt = function() {
    ...
    // Doğru cevabı aktif rastgele soru üzerinden kontrol ediyoruz
    ...
    if (kullaniciCevabi === dogruCevap) {
        ...
        if(mevcutSeviyeIndeksi >= bulmacaVeritabani.length) {
            setTimeout(() => {
                alert("🎉 GÖREV BAŞARILI KAPTAN! Tüm parçalar toplandı, laboratuvara dönüyoruz!");
            }, 1000);
        }
